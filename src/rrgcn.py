...
class RecurrentRGCN(nn.Module):
    def __init__(self, decoder, encoder, gnn, num_ents, num_rels, num_static_rels, num_words, h_dim, opn, sequence_len, num_bases=-1, hidden_size=None,num_basis=-1,
        ...
        self.fuzzy = FuzzyLogicModule(h_dim,6,1)
    ...
    def forward(self, g_list, static_graph=None, device=None):
        gate_list = []
        degree_list = []
        ent_emb = self.seq(self.dynamic_emb)
        self.en_embedding = F.normalize(ent_emb) if self.layer_norm else ent_emb


        if self.use_static:
            static_graph = static_graph.to(device)
            static_graph.ndata['h'] = torch.cat((self.en_embedding, self.words_emb), dim=0)  # 演化得到的表示，和wordemb满足静态图约束
            self.statci_rgcn_layer(static_graph, [])
            static_emb = static_graph.ndata.pop('h')[:self.num_ents, :]
            static_emb = F.normalize(static_emb) if self.layer_norm else static_emb
            self.h = static_emb
        else:
            self.h = F.normalize(self.en_embedding) if self.layer_norm else ent_emb[:, :]
            static_emb = None

        # inverse relation embeddings
        r_rel_emb = torch.mm(self.emb_rel, self.w1) + self.bias_r
        # concatenate relation embeddings
        rel_emb = torch.cat((self.emb_rel, r_rel_emb), dim=0)
        rel_emb = torch.mm(rel_emb, self.w2)
        rel_emb = F.normalize(rel_emb) if self.layer_norm else rel_emb
        history_embs = []

        # self.rel_embedding =  extended_rel_emb
        ...
        for i, g in enumerate(g_list):
            ...
            if i == 0:
                x_input = torch.cat((rel_emb[0:self.num_rels *2], x_input), dim=1)
                # xinzeng
                # x_input = self.fuzzy(x_input)
                self.h_0 = self.relation_cell_1(x_input, rel_emb[0:self.num_rels *2])    # 第1层输入
                self.h_0 = F.normalize(self.h_0) if self.layer_norm else self.h_0
                # # xinzeng
                self.h_0 = self.fuzzy(self.h_0)
                self.h_0 = F.normalize(self.h_0) if self.layer_norm else self.h_0
            else:
                x_input = torch.cat((rel_emb[0:self.num_rels *2], x_input), dim=1)
                # xinzeng
            ...
            x_input = x_input.cpu()
            g = g.cpu()
            static_graph = static_graph.cpu()
        return history_embs, static_emb, self.h_0, gate_list, degree_list


    def forward_train(self, g_list, static_graph=None, device=None):
        gate_list = []
        degree_list = []
        ent_emb = self.seq(self.dynamic_emb)
        self.en_embedding = F.normalize(ent_emb) if self.layer_norm else ent_emb
    ...
    def get_loss(self, glist, triples, static_graph, device):
        ...
        evolve_embs, static_emb, r_emb, _, _ = self.forward_train(glist, static_graph, device)
