...
class HRGNN(nn.Module):
    def __init__(self, graph, num_nodes, num_rels, time_length, time_idx,hidden_size,h_dim, out_dim, max_length=10, a_layer_num=2,
        ...
        self.ori = ori
        self.norm = norm
        self.fuzzy = FuzzyLogicModule(h_dim,5,2)
        if self.norm:
    ...
    def forward(self, data_list, node_id_new=None, time_gap=None,pre_emb=None,r_emb=None, device=None, mode='test'):
        out_triple = data_list['triple']
        self.en_embedding.weight.data  = pre_emb
        pad_row = torch.zeros(1, r_emb.size(1), device=r_emb.device)
        r_emb =  torch.cat((r_emb, pad_row), 0)
        self.rel_embedding.weight.data = r_emb
        ...
        if self.decoder == 'regcn':
            ...
            # etype = sub_d_graph.edata['etype']
            # invalid_indices = torch.where(etype > 511)[0]
            # if len(invalid_indices) > 0:
            #     print(f"Found {len(invalid_indices)} invalid relation indices:")
            #     print("Invalid indices:", invalid_indices)
            #     print("Corresponding etype values:", etype[invalid_indices])
            sub_d_graph.edata['r_h'] = self.rel_embedding(sub_d_graph.edata['etype']) + self.pos_decoder(
                sub_d_graph.edata['e_r'])
            # # xinzeng mohu
            # sub_d_graph.edata['r_h'] = self.fuzzy(sub_d_graph.edata['r_h'])
    ...
    def forward_train(self, data_list, node_id_new=None, time_gap=None,pre_emb=None,r_emb=None, device=None, mode='test'):
        out_triple = data_list['triple']
        self.en_embedding.weight.data  = pre_emb
        pad_row = torch.zeros(1, r_emb.size(1), device=r_emb.device)
        r_emb =  torch.cat((r_emb, pad_row), 0)
        ...
        if self.decoder == 'regcn':
            ...
            sub_d_graph.edata['r_h'] = self.rel_embedding(sub_d_graph.edata['etype']) + self.pos_decoder(
                sub_d_graph.edata['e_r'])
            # # xinzeng mohu
            # sub_d_graph.edata['r_h'] = F.normalize(self.fuzzy(sub_d_graph.edata['r_h']))
