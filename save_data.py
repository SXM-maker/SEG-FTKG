...
if __name__ == '__main__':
    ...
    # 修改后（正确）：
    train_path = 'TKG/data/' + args.data + '/train/'
    valid_path = 'TKG/data/' + args.data + '/val/'
    test_path = 'TKG/data/' + args.data + '/test/'
    mkdir(train_path)
    mkdir(valid_path)
    mkdir(test_path)
    print('Start loading train set: ', datetime.datetime.now(), '=============================================')
    for i, train_data_list in tqdm(enumerate(train_dataset)):
    ...
    for i, val_data_list in tqdm(enumerate(val_dataset)):
    ...
    for i, test_data_list in tqdm(enumerate(test_dataset)):
